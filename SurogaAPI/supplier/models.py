...
class NUTSRegion(models.Model):
    """
    Stores all NUTS hierarchy levels for a postal code
    """
    postal_code = models.CharField(max_length=20, db_index=True)
    country_code = models.CharField(max_length=2, db_index=True)
    nuts0 = models.CharField(max_length=2, verbose_name="Country Code")  # e.g., PT
    nuts1 = models.CharField(max_length=3, verbose_name="Major Socio-Economic Region")  # e.g., PT1
    nuts2 = models.CharField(max_length=4, verbose_name="Basic Region for Policies")  # e.g., PT11
    nuts3 = models.CharField(max_length=5, verbose_name="Small Regions")  # e.g., PT111
    
    class Meta:
        unique_together = [('postal_code', 'country_code')]
        verbose_name_plural = "NUTS Regions"
        indexes = [
            models.Index(fields=['postal_code', 'country_code']),
            models.Index(fields=['nuts0']),
            models.Index(fields=['nuts1']),
            models.Index(fields=['nuts2']),
            models.Index(fields=['nuts3']),
        ]
...
class contractsection(models.Model):
    ...
    - (Canceled) Country: A ForeignKey to the country model specifying the country of operation or relevance for the section. It can be null.
    - (Canceled) DistrictRegion: A ForeignKey to the districtregion model; the region or district mentioned in the section. It can be null.
    - (Canceled) CityMunicipality: A ForeignKey to the citymunicipality model; the city or municipality covered by the section. It can be null.
    - nuts0: Country (NUTS 0)
    - nuts1: Major Region (NUTS 1)
    - nuts2: Policy Region (NUTS 2)
    - nuts3: Small Region (NUTS 3)
    ...
    nuts0 = models.CharField(
        max_length=2, 
        blank=True, 
        null=True,
        verbose_name="Country (NUTS 0)"
    )
    nuts1 = models.CharField(
        max_length=3, 
        blank=True, 
        null=True,
        verbose_name="Major Region (NUTS 1)"
    )
    nuts2 = models.CharField(
        max_length=4, 
        blank=True, 
        null=True,
        verbose_name="Policy Region (NUTS 2)"
    )
    nuts3 = models.CharField(
        max_length=5, 
        blank=True, 
        null=True,
        verbose_name="Small Region (NUTS 3)"
    )
