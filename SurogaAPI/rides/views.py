...
from core.models import publicevent, localwifi, equipment, image, sound, RideDuration, country, districtregion, citymunicipality
#,  commchannel
...
class rideViewSet(viewsets.ModelViewSet): 
    ...
    def createTEST(self, request, *args, **kwargs):
        ...
        if not StreetName:
            return Response({"Success": 0, "Error": 'L000', "Message": "Street name required by Suroga API","ride": serializer.data,"stock":[]})
        ...
        if not CountryName:
            return Response({"Success": 0, "Error": 'L001', "Message": "Country required by Suroga API","ride": serializer.data,"stock":[]})

        DistrictRegionName = serializer.validated_data['DistrictRegion']
        if not DistrictRegionName:
            return Response({"Success": 0, "Error": 'L002', "Message": "District or region name required by Suroga API","ride": serializer.data,"stock":[]})

        CityMunicipalityName = serializer.validated_data['CityMunicipality']
        if not CityMunicipalityName:
            return Response({"Success": 0, "Error": 'L003', "Message": "City and municipality name required by Suroga API","ride": serializer.data,"stock":[]})

        try:
            CountryID = country.objects.get(Name = CountryName)
        except DoesNotExist:
            return Response({"Success": 0, "Error": 'L011', "Message": "Country isn't covered by Suroga API", "ride": serializer.data,"stock":[]})

        try:
            DistrictRegionID = districtregion.objects.get(Name = DistrictRegionName)
        except DoesNotExist:
            return Response({"Success": 0, "Error": 'L012', "Message": "District or region isn't covered by Suroga API", "ride": serializer.data,"stock":[]})

        try:
            CityMunicipalityID = citymunicipality.objects.get(Name = CityMunicipalityName)
        except DoesNotExist:
            return Response({"Success": 0, "Error": 'L013', "Message": "City or municipality isn't covered by Suroga API", "ride": serializer.data,"stock":[]})
        ...
        if len(stockMkt) == 0:
            return Response({"Success": 0, "Error": 'S020', "Message": "Service isn't available by Suroga API for your location.", "ride": serializer.data,"stock":[]})
...
class bookingViewSet(viewsets.ModelViewSet):
    ...
    def send(self, request, pk=None):
        ...
        param = request.query_params.get('by_client')
        by_client = param.lower() == "true" if param is not None else False

        booking = self.get_object()
        board = MessageBoard()
        board.user = booking.user
        board.SendByClient = by_client
        board.Text = message
